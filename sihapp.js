...
function startAIAnalysis() {
  ...
  setTimeout(() => {
    const categories = ['Pothole', 'Street Light', 'Garbage Overflow', 'Water Leak', 'Broken Sidewalk'];
    const detectedType = categories[Math.floor(Math.random() * categories.length)];
    ...
    if (detectedTypeEl) detectedTypeEl.textContent = detectedType;
    if (confidenceEl) confidenceEl.textContent = `${confidence}%`;
    if (severityEl) severityEl.textContent = severity;
    
    AppState.reportData.detectedType = detectedType;
    AppState.reportData.confidence = confidence;
    AppState.reportData.severity = severity;
...
function proceedToSubmit() {
  ...
  if (AppState.recordedBlob) {
    AppState.reportData.voiceNote = 'Voice recording attached';
  } else if (textValue) {
